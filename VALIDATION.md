...
- All 11 XCTest methods: protocol reducers, metadata, Telnet byte boundaries, ARNetwork
  framing, FU-A/STAP-A handling, malformed/truncated packets, assembly limits and recovery,
  split HTTP/SDP, argument validation, non-overwriting recording and UDP loopback.
...
- Complete `scripts/test.sh --desktop` run passed (11 unit tests, smoke test,
  headless integration, failure paths and desktop integration).
...
- Desktop screenshot inspected visually: video, telemetry, controls and logs visible
  without overlap or clipping at 1180 × 780.
...
- Direct-drone/Jumping Sumo transport, flight control and Apple GPU processing features.
